...
function fillPage() {
    ...
    pageData.forEach(item => {
        console.log(item)
        $('tbody').append(`<tr><th scope="row">${count}</th><td>${item.type}</td><td><a href='${item.url}'>${item.url}</a></td><td>${item.created_at}</td><td>${item.company}</td><td><a href='${item.company_url}'>${item.company_url}</a></td><td>${item.location}</td><td>${item.title}</td><td><button onclick='btnDescp(${count})' type="button" class="btn btn-primary" data-toggle="modal" data-target="#exampleModal">Description</button></td><td>${item.how_to_apply}</td><td><img src='${item.company_logo}' height='100px' width='100px'/></td></tr>`)
        count++
    })
